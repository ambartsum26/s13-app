...
(async()=>{
 ...
 assert.equal(d.get('auth-email').hidden,true);assert.equal(d.get('auth-email').value,'ambartsum.tigran@gmail.com');
 callback(null);assert.deepEqual(access,[false]);assert.equal(d.document.body.dataset.authState,'locked');
 auth.currentUser={uid:'8JAUBlCS2CXO0xTJzY1cnOafzuE2'};callback(auth.currentUser);assert.equal(d.document.body.dataset.authState,'locked');assert.deepEqual(access,[false]);assert.match(d.get('auth-message').textContent,/нет доступа/);
 auth.currentUser={uid:'fJT9srxZezNyAVC1NO2Rm6jdK4G3'};callback(auth.currentUser);assert.equal(d.document.body.dataset.authState,'owner');assert.deepEqual(access,[false,true]);
 ...
 d.get('auth-password').value='synthetic-test';await d.get('auth-form').handlers.submit({preventDefault(){}});assert.equal(loginCalls,1);assert.equal(loginEmail,'ambartsum.tigran@gmail.com');assert.equal(loginPassword,'synthetic-test');assert.equal(d.get('auth-password').value,'');assert.equal(d.get('auth-submit').disabled,false);assert.match(d.get('auth-message').textContent,/Проверьте пароль/);
